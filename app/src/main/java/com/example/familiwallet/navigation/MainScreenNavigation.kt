...
@Composable
fun MainScreenNavigation(navigation: NavHostController, modifier: Modifier = Modifier) {
    AnimatedNavHost(
        navController = navigation,
        startDestination = Screen.StartScreen.route,
        enterTransition = { slideIntoContainer(getSlideDirection(initialState, targetState), animationSpec = tween(500)) }
    ) {
        composable(route = Screen.StartScreen.route) { StartScreen(modifier, navigation) }
        ...
        composable(route = Screen.TransactionScreen.route) { TransactionScreen(modifier, navigation) }
    }
}
